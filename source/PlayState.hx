...
class PlayState extends MusicBeatState
{
	...
	var darkstage:FlxTypedGroup<FlxSprite>;
	...
	var fancyspeakers:FlxSprite;
	var heyTimer:Float;
	var frontcrowd1:FlxSprite;
	var backcrowd1:FlxSprite;
	var mic:FlxSprite;
	var cutanim:FlxSprite;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'fancystage':
			{
				...
				var bg:FlxSprite = new FlxSprite(-450, -100).loadGraphic(Paths.image('fancy/fancynightbg'));
				...
				var bgFancywalls:FlxSprite = new FlxSprite(-350, -100).loadGraphic(Paths.image('fancy/fancywalls'));
				bgFancywalls.antialiasing = true;
				bgFancywalls.scrollFactor.set(0.5, 0.5);
				bgFancywalls.active = false;
				bgFancywalls.setGraphicSize(Std.int(bgFancywalls.width * 0.8));
				bgFancywalls.updateHitbox();
				add(bgFancywalls);

				backcrowd = new FlxSprite(-300, 300);
				backcrowd.frames = Paths.getSparrowAtlas('fancy/backcrowdbop');
				backcrowd.animation.addByPrefix('bop', 'Bottom Level Boppers', 24, false);
				backcrowd.antialiasing = true;
				backcrowd.scrollFactor.set(0.5, 0.5);
				backcrowd.setGraphicSize(Std.int(backcrowd.width * 0.5));
				backcrowd.updateHitbox();
				add(backcrowd);

				frontcrowd = new FlxSprite(-200, 500);
				frontcrowd.frames = Paths.getSparrowAtlas('fancy/frontcrowdbop');
				frontcrowd.animation.addByPrefix('bop', "Upper Crowd Bob", 24, false);
				frontcrowd.antialiasing = true;
				frontcrowd.scrollFactor.set(0.5, 0.5);
				frontcrowd.setGraphicSize(Std.int(frontcrowd.width * 0.5));
				frontcrowd.updateHitbox();
				add(frontcrowd);

				//var fgStage:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('fancy/fancystage'));
				var fgStage:FlxSprite = new FlxSprite(-600, -100).loadGraphic(Paths.image('fancy/fancystagealt'));
				...
				if(!ClientPrefs.lowQuality){
					frontcrowd.animation.play('bop', true);
					backcrowd.animation.play('bop', true);
				}
			
			}
			case 'fancystagedark':
			{
				curStage = 'fancystagedark';

				defaultCamZoom = 0.80;

				var bg1:FlxSprite = new FlxSprite(-450, -100).loadGraphic(Paths.image('fancy/fancynightbg'));
				...
				var bgFancywalls1:FlxSprite = new FlxSprite(-350, -100).loadGraphic(Paths.image('fancy/fancywallsalt'));
				bgFancywalls1.antialiasing = true;
				bgFancywalls1.scrollFactor.set(0.5, 0.5);
				bgFancywalls1.active = false;
				bgFancywalls1.setGraphicSize(Std.int(bgFancywalls1.width * 0.8));
				bgFancywalls1.updateHitbox();
				add(bgFancywalls1);

				backcrowd1 = new FlxSprite(-300, 300);
				backcrowd1.frames = Paths.getSparrowAtlas('fancy/backcrowdbop');
				backcrowd1.animation.addByPrefix('bop', 'Bottom Level DARK', 24, false);
				backcrowd1.antialiasing = true;
				backcrowd1.scrollFactor.set(0.5, 0.5);
				backcrowd1.setGraphicSize(Std.int(backcrowd1.width * 0.5));
				backcrowd1.updateHitbox();
				add(backcrowd1);

				frontcrowd1 = new FlxSprite(-200, 500);
				frontcrowd1.frames = Paths.getSparrowAtlas('fancy/frontcrowdbop');
				frontcrowd1.animation.addByPrefix('bop', "Upper Crowd DARK", 24, false);
				frontcrowd1.antialiasing = true;
				frontcrowd1.scrollFactor.set(0.5, 0.5);
				frontcrowd1.setGraphicSize(Std.int(frontcrowd1.width * 0.5));
				frontcrowd1.updateHitbox();
				add(frontcrowd1);

				//var fgStage:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('fancy/fancystage'));
				var fgStage1:FlxSprite = new FlxSprite(-600, -100).loadGraphic(Paths.image('fancy/fancystagealt'));
				fgStage1.active = false;
				fgStage1.antialiasing = true;
				add(fgStage1);

				if(!ClientPrefs.lowQuality){
					frontcrowd1.animation.play('bop', true);
					backcrowd1.animation.play('bop', true);
				}
			}
			case 'finalstage':
			{
				...
				darkstage = new FlxTypedGroup<FlxSprite>();
				add(darkstage);

				var bg:FlxSprite = new FlxSprite(-450, -100).loadGraphic(Paths.image('fancy/fancynightbgdark'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.2, 0.2);
				bg.active = false;
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				...
				var bgFancywalls:FlxSprite = new FlxSprite(-350, -100).loadGraphic(Paths.image('fancy/fancywallsdark'));
				bgFancywalls.antialiasing = true;
				bgFancywalls.scrollFactor.set(0.5, 0.5);
				bgFancywalls.active = false;
				bgFancywalls.setGraphicSize(Std.int(bgFancywalls.width * 0.8));
				bgFancywalls.updateHitbox();
				darkstage.add(bgFancywalls);

				var darktables:FlxSprite = new FlxSprite(-300, 300).loadGraphic(Paths.image('fancy/darktables'));
				darktables.antialiasing = true;
				darktables.scrollFactor.set(0.5, 0.5);
				darktables.active = false;
				darktables.setGraphicSize(Std.int(darktables.width * 0.5));
				darktables.updateHitbox();
				darkstage.add(darktables);

				var fgStage:FlxSprite = new FlxSprite(-600, -100).loadGraphic(Paths.image('fancy/fancystagedark'));
				fgStage.active = false;
				fgStage.antialiasing = true;
				darkstage.add(fgStage);

				var stagemic:FlxSprite = new FlxSprite(80, 435).loadGraphic(Paths.image('mic'));
				stagemic.antialiasing = true;
				stagemic.updateHitbox();
				darkstage.add(stagemic);

				if (isStoryMode)
					{
						for (spr in darkstage) {
							spr.alpha = 0.0001;
						}
					}
				if (seenCutscene)
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'chocolates' | 'lovesick':
					if(daSong == 'chocolates') FlxG.sound.playMusic(Paths.music('LovelyNight'), 0);
				FlxG.sound.music.fadeIn(1, 0, 0.8);
					if(daSong == 'lovesick') FlxG.sound.playMusic(Paths.music('LovelyNightSlow'), 0);
				FlxG.sound.music.fadeIn(1, 0, 0.8);
				...
				case 'jilted':
					...
					cutanim = new FlxSprite(-50, 220);
					cutanim.frames = Paths.getSparrowAtlas('CupidDefeatSheet');
					cutanim.animation.addByPrefix('bop', 'CupidDefeat', 24, false);
					cutanim.antialiasing = true;
					cutanim.scale.set(0.45,0.45);
					cutanim.alpha = 0;
					cutanim.updateHitbox();
					cutanim.x += 130;
					cutanim.y += 35;
					add(cutanim);
					FlxTween.tween(cutanim, {alpha: 0}, 1, {
						ease: FlxEase.linear,
						onComplete: function(twn:FlxTween) {
							cutanim.alpha = 1;
							FlxG.sound.play(Paths.sound('cupiddeath'));
							cutanim.animation.play('bop');
							mic.visible = true;
						}
					});

					mic = new FlxSprite(cutanim.x, cutanim.y + 180).loadGraphic(Paths.image('mic'));
					mic.antialiasing = true;
					mic.visible = false;
					mic.updateHitbox();
					add(mic);

					blackShit1 = new FlxSprite(-FlxG.width * FlxG.camera.zoom,
						-FlxG.height * FlxG.camera.zoom).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.WHITE);
					blackShit1.scrollFactor.set();
					blackShit1.alpha = 0.0001;
					add(blackShit1);

					cutanim.animation.callback = function(bop, frameNumber:Int, frameIndex:Int)
						{
							if (frameNumber == 18)
								{
									for (spr in darkstage) {
										FlxTween.tween(spr, {alpha: 1}, 0.6, {ease: FlxEase.expoOut});
										FlxTween.tween(boyfriend, {alpha: 1}, 0.6, {ease: FlxEase.expoOut});
										FlxTween.tween(gf, {alpha: 1}, 0.6, {ease: FlxEase.expoOut});
										FlxTween.tween(camFollow, {y: camFollow.y - 50}, 0.6, {ease: FlxEase.expoOut});
										FlxTween.tween(camFollow, {x: camFollow.x + 250}, 0.6, {ease: FlxEase.expoOut});
									}
								}
						}

					cutanim.animation.finishCallback = function(pop)
						{
							FlxTween.tween(blackShit1, {alpha: 1}, 1, {ease: FlxEase.quadInOut,
								onComplete: function(twn:FlxTween)
								{
									if(daSong == 'jilted') FlxG.sound.playMusic(Paths.music('BleedingHeart'), 0);
									FlxG.sound.music.fadeIn(1, 0, 0.8);
									startDialogue(dialogueJson);
								}
							});
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			if (songName == 'jilted' && seenCutscene && !seen)
				{
					seen = true;
					iconP1.alpha = 1;
					iconP2.alpha = 1;
					...
					dad.alpha = 1;
					camHUD.visible = true;
					FlxTween.tween(blackShit1, {alpha: 0}, 0.25, {ease: FlxEase.quadInOut,
						onComplete: function(twn:FlxTween)
						{
							remove(blackShit1);
						}
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				if(curStage == 'fancystage' || curStage == 'fancystagedark') {
					//frontcrowd.dance(true);
					//backcrowd.dance(true);
				}
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (songName != 'tutorial')
					{
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/ending_slideshow.webm', new StoryMenuState()));
					}
					else
					{
